...
package Game.Entity
{
   ...
   public class EnemyAI extends GameEntity
   {
      ...
      override public function update(_dt:Number):void
      {
         ...
         if (actionDelay <= 0)
         {
            if (team == 6)
               ...
               actionDelay = Math.max(0.1, (3 - Globals.currentDifficulty) * (1 + Math.random() * 1));
         }
      ...
      public function attackBasic(_dt:Number):void // 进攻
      {
         var _Node:Node = null;
         sender = [];
         targets = [];
      ...
      public function defendBasic(_dt:Number):void // 回防
      {
      ...
      public function moveBasic(_senderNode:Node, _targetNode:Node, _ships:Number):String // 移动
      {
         if (_senderNode == _targetNode)
            return null; // 不能移动到自己身上
         if (_senderNode.type == 1)
            _senderNode.sendAIShips(team, _targetNode, _ships);
         else
         {
         ...
         return _senderNode.tag + " move to " + _targetNode.tag + " with " + _ships + " ships";
      }
      // #endregion
      // #region 计算工具
      // 判断路径是否被拦截并计算拦截点
